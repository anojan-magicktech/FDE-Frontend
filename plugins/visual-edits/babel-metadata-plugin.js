...
const babelMetadataPlugin = ({ types: t }) => {
  ...
  const TRACING_STACK = new Set(); // Cycle detection for prop tracing
  ...
  function tracePropToSource(propName, componentName, exprPath, state) {
    ...
    if (TRACING_STACK.has(traceKey)) return null;
    TRACING_STACK.add(traceKey);
