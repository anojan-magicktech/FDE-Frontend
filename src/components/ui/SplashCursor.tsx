...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function resizeCanvas() {
      ...
      const width = scaleByPixelRatio(w);
      const height = scaleByPixelRatio(h);
    ...
    function hexToRGB(hex: string): ColorRGB {
      ...
      return { r: r * 0.45, g: g * 0.38, b: b * 0.12 };
    }
    ...
    const handleFirstTouchStart = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        const pos = getCanvasPos(touches[i].clientX, touches[i].clientY);
        if (pos) {
          updateFrame();
