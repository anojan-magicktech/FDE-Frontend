...
import { ArrowRight, X, Calendar, MapPin, User, Tag, Star, ThumbsUp, Quote, Search, Filter } from 'lucide-react';
...
// Cache projects data in memory to prevent layout shifts and stutters on route navigation
let cachedProjects = null;

export const Projects = ({ isPreview = false, hideHeader = false }) => {
    ...
    const uniqueServices = ['All', ...new Set(projects.map(p => p.service_type_details?.title).filter(Boolean))];
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {displayedProjects.map((project, index) => (
                            ...
                            >
                                {project.status && (
                                    <div className={`absolute top-4 right-4 z-20 px-3.5 py-1 text-[10px] font-bold uppercase tracking-widest shadow-md backdrop-blur-md rounded-full border ${
                                        project.status === 'Completed' ? 'text-green-400 bg-black/80 border-green-500/20' :
                                        project.status === 'In Process' ? 'text-royal-blue-bright bg-black/80 border-blue-500/20' :
                                            ...
                                            <div className="flex flex-wrap gap-6 mb-8 text-white/50 font-body text-xs sm:text-sm border-y border-white/5 py-6">
                                                ...
                                                {selectedProject.date && (
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span>Date: <span className="text-white font-medium">{selectedProject.date}</span></span>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
